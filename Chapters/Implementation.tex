...
. We used \cc{with_updated_state} to create rules to updated nested versions of the indentation (\cc{n} and \cc{m} in the YAML spec) and context (\cc{c} in the YAML spec). Using this approach we were able to translate almost any rule without too much problems.

For the conversion of the parsed data we used the AST building facility provided by PEGTL and custom tree walking code similar to the one used in the \href{https://www.libelektra.org/plugins/yawn}{YAwn} plugin (see Section “\nameref{sec:earley_parser}”).
