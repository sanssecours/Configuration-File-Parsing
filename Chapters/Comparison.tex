...
Figure~\ref{fig:line_count} shows the results reported by \FileCountLines{}. As you can see we did not have to write a grammar for the library based YAML CPP plugin. For the other plugins, we either created a grammar (Yan LR, YAMBi, YAwn), or specified the grammar as handwritten code (Yay PEG). Only the \gls{ANTLR} (Yan LR) and Bison (YAMBi) based plugins use generated code. YAwn on the other hand, which is based on \gls{YAEP}, uses the grammar directly.

\begin{figure}[H]
  \begin{bchart}[max=2100, width=0.8\textwidth, unit={~Lines of Code}]
...
For the plugins directly based on a parser, we had to write the least code for Yan LR, followed by YAMBi, YAwn, and YAy PEG. The line count numbers for YAMBi and YAwn are not that surprising, considering that we implemented some code functionality already provided by ANTLR, for those plugins ourselves.
...
\begin{itemize}
  ...
  \item classes storing positioning information ($38$ code lines)
\end{itemize}

. The sum of the code lines ($552$) mentioned above is responsible for about half of the code lines of the whole plugin ($\frac{1096}{2} = 548$).

YAy PEG, with its scanner-less parsing engine based on C++ templates, uses the most handwritten code. Just like for YAwn we added
...
for the plugin. The other lines of code take care of parsing ($822$ code lines) and the communication between the plugin and Elektra ($115$ code lines). The amount of parsing code might look pretty large, if we compare it to the grammar code of the other plugins. However, we have to consider that
