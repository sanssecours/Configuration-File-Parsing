...
The source code of LibYAML showed us that we can take care of most of YAML’s  complexity in the lexer. For further information we looked at \href{https://github.com/llvm-mirror/llvm/blob/master/lib/Support/YAMLParser.cpp}{LLVM’s YAML parser} (written in C++) and \href{https://bitbucket.org/asomov/snakeyaml-engine}{SnakeYAML Engine} (written in Java). The lexer of both of these tools use the same basic ideas as LibYAML. However, the code of these libraries is easier to read than LibYAML’s, since they are written in higher level programming languages.
...
Apart from the parsing algorithm, another difference between Yan LR and YAMBi is the approach on how to translate YAML data to a \cpp{KeySet}. For YAN LR we use a listener interface that operates on the parse tree created by ANTLR. Bison only offers parser actions. These actions contain code that will be called after the parser matches certain parts of the grammar.

YAMBi uses parser actions to call methods of a class that stores temporary data used to create the \cpp{KeySet}. The code for this approach is actually not that different from the one of the listener we use for Yan LR. The only problematic part was translating a code path in Yan LR that operates on parsing data that is available via ANTLR’s listener interface, since the listener operates on a completed parse tree, but not available yet in the corresponding grammar rule of the Bison parser. We solved this problem by by adding an additional grammar rule to the Bison grammar and moving the code into a later phase of the parse process.
