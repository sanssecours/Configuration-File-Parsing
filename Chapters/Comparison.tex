...
To compare the run-time performance we used the C application \href{https://master.libelektra.org/benchmarks/plugingetset.c}{\sh{benchmark_plugingetset}} that opens an Elektra plugin using a specific configuration file. For the whole benchmark process we created a Bash script, called \href{https://master.libelektra.org/scripts/benchmark-yaml.in}{\sh{benchmark-yaml}}, that uses the benchmarking tool \href{https://github.com/sharkdp/hyperfine}{\sh{hyperfine}} to call \href{https://master.libelektra.org/benchmarks/plugingetset.c}{\sh{benchmark_plugingetset}} using different YAML plugins. Figure~\ref{fig:benchmark} shows a diagram of this setup.

For the measurement we use \href{https://github.com/sharkdp/hyperfine}{\sh{hyperfine}}, sine this tool

\begin{enumerate}
  \item automatically determines how often it should call \sh{benchmark_plugingetset} for meaningful measurement results,
...
As the first input for the benchmark we used a \href{https://master.libelektra.org/src/plugins/yajl/yajl/keyframes_complex.json}{JSON configuration file} of the \href{https://www.libelektra.org/plugins/yajl}{YAJL plugin} that we converted to block syntax using the \href{https://www.libelektra.org/plugins/yamlcpp}{YAML CPP} plugin. We then modified the exported data by removing all \yaml{!<!elektra/meta>} tags, which are not supported by the other YAML plugins. We call the resulting file \href{https://github.com/ElektraInitiative/rawdata/tree/master/YAML/keyframes.yaml}{\code{keyframes.yaml}} in the remainder of the thesis. This file and all other data of the benchmark is available here:

\begin{leftbar}
  \url{https://github.com/ElektraInitiative/rawdata/tree/master/YAML}
\end{leftbar}

. For another input file called \href{https://github.com/ElektraInitiative/rawdata/tree/master/YAML/combined.yaml}{\code{combined.yaml}} we copy and pasted data from test data and various other YAML files in Elektra’s repository into a single file. While the file content is nonsensical, it should at least contains a mix of YAML data that covers most of the code paths of the YAML plugins.

Since both of these files are relatively small,\href{https://github.com/ElektraInitiative/rawdata/tree/master/YAML/keyframes.yaml}{\code{keyframes.yaml}} contains 218 lines, while \href{https://github.com/ElektraInitiative/rawdata/tree/master/YAML/combined.yaml}{\code{combined.yaml}} contains 152 lines, we also generated data using a Python script that we called \href{https://master.libelektra.org/scripts/generate-yaml}{\code{generate-yaml}}. This script generates YAML maps using \href{https://en.wikipedia.org/wiki/Universally_unique_identifier}{\glspl{UUID}} as scalar keys and values. For the YAML scalars the script randomly selects one of the three flow scalar styles:
...
\begin{itemize}
  \item \href{https://github.com/ElektraInitiative/rawdata/tree/master/YAML/generated.yaml}{\code{generated.yaml}} that contains 10 000 lines, and

  \item \href{https://github.com/sanssecours/rawdata/blob/⏱/YAML/Input/generated_100000.yaml}{\code{generated\_100000.yaml}} that contains 100 000 lines
\end{itemize}

. We also created another script called \href{https://github.com/sanssecours/rawdata/blob/⏱/YAML/Scripts/cut_input}{\code{cut\_input}} to generate additional smaller input files that contain the first 50000, 10000, 5000, 1000, 500, 100, 50, 10, 5 and 1 lines of \href{https://github.com/ElektraInitiative/rawdata/tree/master/YAML/generated.yaml}{\code{generated\_100000.yaml}}. The script also generates a file that contains only the empty string.
...
\begin{figure}[H]
  \begin{bchart}[max=30, width=0.8\textwidth, unit=ms]
    ...
    \bcbar[text=17.5 ± 0.5, value=macOS, color=Aqua]{17.5}
    \bcbar[text=30.2 ± 1.5, value=Linux/GCC, color=Aqua]{30.2}
    \bcbar[text=31.1 ± 3.0, value=Linux/Clang, color=Aqua]{31.1}
    ...
    \bcbar[text=15.4 ± 0.4, value=macOS, color=DarkOrchid]{15.4}
    \bcbar[text=24.6 ± 2.8, value=Linux/GCC, color=DarkOrchid]{24.6}
    \bcbar[text=25.8 ± 2.3, value=Linux/Clang, color=DarkOrchid]{25.8}

    \bcbar[text=27.7 ± 1.8, value=macOS, color=LightGray]{27.7}
    \bcbar[text=24.4 ±1.8, value=Linux/GCC, color=LightGray]{24.4}
    \bcbar[text=26.4 ± 3.3, value=Linux/Clang, color=LightGray]{26.4}
  \end{bchart}
  \begin{center}
  \vspace{-0.5cm}
    \tikzcircle{orange} YAML CPP ~~
    \tikzcircle{Aqua} Yan LR ~~
    \tikzcircle{Turquoise3} YAMBi ~~
    \tikzcircle{DarkOrchid} YAwn ~~
    \tikzcircle{LightGray} YAy PEG
  \vspace{-0.5cm}
  \end{center}
  \caption{This bar chart shows the run time of the plugins for the input \href{https://github.com/ElektraInitiative/rawdata/tree/master/YAML/keyframes.yaml}{\code{keyframes.yaml}}}
...
\begin{figure}[H]
  \begin{bchart}[max=35, width=0.8\textwidth, unit=ms]
    ...
    \bcbar[text=9.8 ± 0.4, value=macOS, color=Aqua]{9.8}
    \bcbar[text=25.2 ± 2.6, value=Linux/GCC, color=Aqua]{25.2}
    \bcbar[text=26.2 ± 2.4, value=Linux/Clang, color=Aqua]{26.2}
    ...
    \bcbar[text=8.4 ± 0.4, value=macOS, color=DarkOrchid]{8.4}
    \bcbar[text=22.3 ± 1.6, value=Linux/GCC, color=DarkOrchid]{22.3}
    \bcbar[text=21.1 ± 1.4, value=Linux/Clang, color=DarkOrchid]{21.1}

    \bcbar[text=14.7 ± 0.6, value=macOS, color=LightGray]{14.7}
    \bcbar[text=29.8 ± 8.1, value=Linux/GCC, color=LightGray]{29.8}
    \bcbar[text=21.5 ± 1.4, value=Linux/Clang, color=LightGray]{21.5}
  \end{bchart}
  \begin{center}
  \vspace{-0.5cm}
    \tikzcircle{orange} YAML CPP ~~
  ...
  \vspace{-0.5cm}
  \end{center}
  \caption{This bar chart shows the run time of the plugins for the input \href{https://github.com/ElektraInitiative/rawdata/tree/master/YAML/combined.yaml}{\code{combined.yaml}}}
...
\begin{figure}[H]
  \begin{bchart}[max=385, width=0.8\textwidth, unit=ms]
    ...
    \bcbar[text=132 ± 3.7, value=macOS, color=Aqua]{132}
    \bcbar[text=132.7 ± 3.4, value=Linux/GCC, color=Aqua]{132.7}
    \bcbar[text=124.5 ± 5.2, value=Linux/Clang, color=Aqua]{124.5}
    ...
    \bcbar[text=81.6 ± 2.9, value=macOS, color=DarkOrchid]{81.6}
    \bcbar[text=79.4 ± 4, value=Linux/GCC, color=DarkOrchid]{79.4}
    \bcbar[text=74.9 ± 3.1, value=Linux/Clang, color=DarkOrchid]{74.9}

    \bcbar[text=381 ± 7, value=macOS, color=LightGray]{381}
    \bcbar[text=160.2 ± 6.6, value=Linux/GCC, color=LightGray]{160.2}
    \bcbar[text=161.1 ± 7.5, value=Linux/Clang, color=LightGray]{161.1}
  \end{bchart}
  \begin{center}
  \vspace{-0.5cm}
    \tikzcircle{orange} YAML CPP ~~
  ...
  \vspace{-0.5cm}
  \end{center}
  \caption{This bar chart shows the run time of the plugins for the input \href{https://github.com/ElektraInitiative/rawdata/tree/master/YAML/generated.yaml}{\code{generated.yaml}}}
...
\begin{figure}[H]
  ...
  \caption{The diagrams above show the runtime of the plugins for the input file \href{https://github.com/sanssecours/rawdata/blob/⏱/YAML/Input/generated_100000.yaml}{\code{generated\_100000.yaml}} and other files that contains only the first $n$ number of lines of this file.}
...
\begin{figure}[H]
  ...
  \caption{The diagrams above shows that the runtime for the first lines of the file \href{https://github.com/sanssecours/rawdata/blob/⏱/YAML/Input/generated_100000.yaml}{\code{generated\_100000.yaml}} almost certainly grows linearly.}
  \label{fig:benchmark_results_generated_above_1000}
\end{figure}

From the correlation coefficients ($R$) of $0.96$ and higher we deduce that the runtime almost certainly grows linearly. This means that the approximate runtime of the plugins should be the same. Figure~\ref{fig:benchmark_generated} shows that the constant difference between the runtime can be relatively high. For example on macOS, the fastest plugin YAwn is nearly five times faster than the slowest plugin YAy PEG for the file \href{https://github.com/ElektraInitiative/rawdata/tree/master/YAML/generated.yaml}{\code{generated.yaml}}:
