...
From the tested parser libraries and generators only ANTLR offers multi-error support without requiring changes to the grammar. The Bison and YAEP parsers on the other hand require manual updates to the grammar to specify error points. These grammar changes also change the interpreted language of these parsers and can cause conflicts (Bison) or ambiguous output (\gls{YAEP}). Overall the error messages produced by the ANTLR, Bison and YAEP parsers are not that great, but comparable to the ones produced by the handwritten recursive descent parser of yaml-cpp. The parser of yaml-cpp does not support error recovery and hence only reports the first syntax error. The library with the most limited built-in error handling capabilities was \gls{PEGTL}. The parser based on this library also showed a common problem among PEG parsers, the inability to always show the correct position of the first syntax error.
