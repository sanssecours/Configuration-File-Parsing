...
\begin{listing}
  ...
  \end{code-boxed}
  \caption{The indentation of the plain scalar \yaml{element 2} is incorrect in the file \sh{list_element_outside.yaml}. One of the most obvious solutions to fix the syntax error would be to add a single space character right before \yaml{element 2}.}
...
\begin{table}
  ...
  \begin{tabular}{lp{12cm}}
    ...
    Yan LR & 3:1: mismatched input \textquotesingle- \textquotesingle\ expecting BLOCK\_END\\
...
As we can see in Table~\ref{tab:error_messages_list_element_outside} all of the parsing engines report the error location for the code from Listing~\ref{lst:list_element_outside} correctly. The error messages also shows that the question, if the first position after a newline is at column 0 or 1, is still open for debate.
