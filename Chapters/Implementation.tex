...
. Out of these options Syck is not actively maintained any more. This leaves only \code{libyaml} and \code{yaml-cpp}. We decided to use \code{yaml-cpp}, since it supports the latest version of YAML (\code{YAML 1.2}). With the help of the library we added the first plugin with full YAML support called \href{https://www.libelektra.org/plugins/yamlcpp}{YAML CPP} to Elektra.
