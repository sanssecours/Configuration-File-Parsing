...
in order. It succeeds if one of the argument matches, or fails if all of them fail. The rule \cc{seq} on the other hand tries to match all it’s template arguments in sequence and either succeeds, if all of them succeed, or fails, if one of them fails. The rules \cc{at} and \cc{not_at} represents the PEG predicate \cc{&} and \cc{!}. The predicate rule \cc{at} (\cc{&}) succeeds if the current input matches the given input and fails if it does not. The predicate rule \cc{not_at} behaves exactly opposite. Neither of the predicates consumes any input. The only remaining rule is \cc{one}, which tries to match any of the given characters.
...
For the conversion of the parsed data to a \cc{KeySet} we used the \gls{AST} building facility provided by PEGTL, which allows us to specify which matched rules should create nodes in the \gls{AST}. Custom tree walking code then walks this tree invoking a listener to create a \cc{KeySet}. This approach is very similar to the one used by the \href{https://www.libelektra.org/plugins/yawn}{YAwn} plugin (see Section “\nameref{sec:earley_parser}”).
