...
\declaretheorem[name={Main Research Question},
                style=questionStyle,
                numbered=no]
               {question}
\declaretheorem[name={Research Question},
                style=questionStyle]
               {subquestion}
...
\newacronym{GLR}{GLR}{Generalized LR}
\newacronym{IELR}{IELR}{Inadequacy Elimination LR}
...
\newglossaryentry{token}{
    name = token,
    description = {Often parsers do not directly work on the character level. Instead a lexer (aka scanner or tokenizer) assembles parts of the text into tokens (aka symbols). The parser then uses a stream of tokens as input instead of a stream of characters}
}
