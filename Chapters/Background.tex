...
Since Elektra orders a \code{KeySet} alphabetically, a \cc{Key} with name \code{\#10} would appear before a \cc{Key} with name \code{\#2}. To fix this problem Elektra adds underscores to keys with larger indices. For example, the 11th element of an array ends with the name \code{\#\_10}.
