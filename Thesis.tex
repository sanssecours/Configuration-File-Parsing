...
\newglossaryentry{collection}{
    name = collection,
    description = {A collection is a data structure used to store multiple items. In \glstext{YAML} the term collection is used to refer to both sequences and mappings}
}

\newglossaryentry{simpleKey}{
    name = simple key,
    description = {A simple \glstext{YAML} key is a key that does not start with the explicit mapping start symbol “\yaml{?}”}
}
