...
For the resource usage we analyze the heap memory consumption and answer~\Cref{que:memory}.

\memory*

To make sure that the plugins are maintainable we analyze code sizes and take a look at the cyclomatic complexity. Using the information gained in this task we answer~\Cref{que:size}.

\size*

We also examine the extensibility and composability of the plugins looking at code changes for specific bug fixes and feature additions. This step helps us to answer~\Cref{que:closeness}.

\closeness*

To make sure the plugins produce good error messages we also compare these messages for specific input files in a detailed analysis and answer~\Cref{que:message}.

\message*

\section{Evaluated Plugins}

The mapping between the parsing plugin names and the parser libraries/generators might be not apparent to everybody. Since we mainly use the plugin name in the evaluation we provide an overview of the mapping in Table~\ref{table:plugins}. The colored text shows how you can memorize the mapping more easily. Apart from YAwn all plugins share at least two consecutive letters with the used parsing library. The name of YAwn is based on the very bad that “pun” that you might have to “yawn” if you get up earl(e)y.
...
\midrule
\tikzcircle{orange} \textcolor{orange}{YAML CPP} & \textcolor{orange}{yaml}-\textcolor{orange}{cpp} & Handwritten Recursive Descent Parser\\
\tikzcircle{DarkYellow} Y\textcolor{DarkYellow}{an LR} & \textcolor{DarkYellow}{AN}T\textcolor{DarkYellow}{LR} & \gls{ALL(*)}\\
\tikzcircle{Turquoise3} YAM\textcolor{Turquoise3}{Bi} & \textcolor{Turquoise3}{Bi}son & \gls{LALR}\\
\tikzcircle{Aqua} \textcolor{Aqua}{YAwn} & \gls{YAEP} & \textcolor{Aqua}{Earl}e\textcolor{Aqua}{y} Parser\\
\tikzcircle{DarkOrchid} YAy \textcolor{DarkOrchid}{PEG} & \textcolor{DarkOrchid}{PEG}TL & PEG Parser\\
\bottomrule
  \end{tabular}
\end{table}
...
\codebox{%
\href{https://github.com/ElektraInitiative/libelektra/blob/5e28fd6e97dfdab2136b2242ba0187df5fa619b9/scripts/generate-yaml}%
...
As first input for the benchmarks we used a \href{\URLKeyFramesJSON}{JSON configuration file} of the \href{https://www.libelektra.org/plugins/yajl}{YAJL plugin} that we converted to block syntax using the \LinkYAMLCPP{} plugin. We then modified the exported data by removing all \yaml{!<!elektra/meta>} tags, which are not supported by the other \glstext{YAML} plugins. We call the resulting file \FileKeyFrames{} in the remainder of the thesis. This file and all other data of the benchmarks is available here:

\begin{leftbar}
  \url{http://rawdata.libelektra.org/tree/master/YAML}
\end{leftbar}

For another input file called \FileCombined{} we copy and pasted parts of test data and various other \glstext{YAML} files in Elektra’s repository into a single file. While the file content is nonsensical, it should at least contain a mix of \glstext{YAML} data that covers most of the code paths of the \glstext{YAML} plugins.
...
This always works since \glspl{UUID} contain no character sequence that has special meaning according to the \glstext{YAML} specification. Using this method we generated two files composed of mappings and scalars.

\begin{itemize}
  \item The file \FileGenerated{} contains 10 000 lines and has a maximum nesting of 26 levels.

  \item The file \FileGeneratedHundredThousand{} contains 100 000 lines and has a maximum nesting of 31 levels.
\end{itemize}
...
\begin{figure}[H]
  \centering
    \includegraphics[width=0.87\textwidth]{Benchmark}
...
\begin{itemize}
  ...
  \item Even YAy PEG’s backtracking parser without memoization shows linear behavior. As we already mentioned before in “\nameref{sec:state_of_the_art}” it is generally not clear, if memoization provides runtime improvements for a certain grammar. This seems also one of the reasons, why \gls{PEGTL} does not use memoization, as can be seen in a quote of Colin Hirsch~\cite{hirsch2016memo}, one of the authors of \gls{PEGTL}, below.
...
\begin{enumerate}
  \item The difference between the runtime of the fastest and slowest plugin for large files is nearly twice as large on macOS.
...
We determined with a high confidence that all of the \glstext{YAML} plugins show a linear runtime behavior (see Figure~\ref{fig:benchmark_results_generated_above_1000}), at least for the file \FileGeneratedHundredThousand{}. This puts all plugins, into the same computational complexity class. The constant factors between the runtimes can still be relatively high as we can see in Equation~\ref{eq:benchmark_difference}.
...
\begin{figure}[H]
  \begin{bchart}[max=1500, width=0.8\textwidth, unit=KB]
    \bcbar[text=406, value=Linux/GCC, color=orange]{406}
    \bcbar[text=406, value=Linux/Clang, color=orange]{406}

    \bcbar[text=747, value=Linux/GCC, color=DarkYellow]{747}
    \bcbar[text=747, value=Linux/Clang, color=DarkYellow]{747}

    \bcbar[text=295, value=Linux/GCC, color=Turquoise3]{295}
    \bcbar[text=295, value=Linux/Clang, color=Turquoise3]{295}

    \bcbar[text=1428, value=Linux/GCC, color=Aqua]{1428}
    \bcbar[text=1429, value=Linux/Clang, color=Aqua]{1429}

    \bcbar[text=363, value=Linux/GCC, color=DarkOrchid]{363}
    \bcbar[text=363, value=Linux/Clang, color=DarkOrchid]{363}
  \end{bchart}
  \begin{center}
  \vspace{-0.5cm}
    \tikzcircle{orange} YAML CPP ~~
...
\begin{figure}[H]
  \begin{bchart}[max=1300, width=0.8\textwidth, unit=KB]
    \bcbar[text=245, value=Linux/GCC, color=orange]{245}
    \bcbar[text=245, value=Linux/Clang, color=orange]{245}

    \bcbar[text=431, value=Linux/GCC, color=DarkYellow]{431}
    \bcbar[text=431, value=Linux/Clang, color=DarkYellow]{431}

    \bcbar[text=208, value=Linux/GCC, color=Turquoise3]{208}
    \bcbar[text=208, value=Linux/Clang, color=Turquoise3]{208}

    \bcbar[text=1232, value=Linux/GCC, color=Aqua]{1232}
    \bcbar[text=1232, value=Linux/Clang, color=Aqua]{1232}

    \bcbar[text=219, value=Linux/GCC, color=DarkOrchid]{219}
    \bcbar[text=219, value=Linux/Clang, color=DarkOrchid]{219}
  \end{bchart}
  \begin{center}
  \vspace{-0.5cm}
    \tikzcircle{orange} YAML CPP ~~
...
\begin{figure}[H]
  \begin{bchart}[max=25, width=0.8\textwidth, unit=MB]
    \bcbar[text=9.8, value=Linux/GCC, color=orange]{9.8}
    \bcbar[text=9.8, value=Linux/Clang, color=orange]{9.8}

    \bcbar[text=24.5, value=Linux/GCC, color=DarkYellow]{24.5}
    \bcbar[text=24.5, value=Linux/Clang, color=DarkYellow]{24.5}

    \bcbar[text=10.5, value=Linux/GCC, color=Turquoise3]{10.5}
    \bcbar[text=10.5, value=Linux/Clang, color=Turquoise3]{10.5}

    \bcbar[text=20.9, value=Linux/GCC, color=Aqua]{20.9}
    \bcbar[text=20.9, value=Linux/Clang, color=Aqua]{20.9}

    \bcbar[text=8.6, value=Linux/GCC, color=DarkOrchid]{8.6}
    \bcbar[text=8.6, value=Linux/Clang, color=DarkOrchid]{8.6}
  \end{bchart}
  \begin{center}
  \vspace{-0.5cm}
    \tikzcircle{orange} YAML CPP ~~
...
We conclude the subsection about the memory evaluation, by answering ~\Cref{que:memory}.

\memory*

While the asymptotic memory usage of all plugins grows linearly according to our measurements, the factors between the memory usages can be quite high. If we look at the data for the file \FileGenerated{} (see Figure~\ref{fig:benchmark_memory_generated}), then YAy PEG performs best, while YAML CPP needs about $14\%$, and YAMBi needs about $22\%$  more heap memory.  The memory usage of the other plugins is much worse. YAwn takes about $2.4$ times the memory amount of YAy PEG, and Yan LR needs even more than $2.8$ times the heap memory of YAy PEG.
...
Figure~\ref{fig:line_count} shows the individual line counts and therefore answers the first part of~\Cref{que:size}.

\size*

To answer the second part of the question we have to analyze the individual code sizes further.

Figure~\ref{fig:line_count} show that YAML CPP requires the least amount of code. This results is not surprising, considering that the plugin uses yaml-cpp, a library that already represents the \glstext{YAML} file using high-level abstract data structures. We can convert these data structures into Elektra’s \cpp{KeySet} structure relatively easily, keeping the code size of the plugin small.
...
If we look at the other YAML plugins, we had to write the least code for Yan LR, followed by YAMBi, YAwn, and YAy PEG. The higher line counts of YAMBi and YAwn compared to Yan LR are not that surprising, considering that we implemented some functionality already provided by \gls{ANTLR}, for those plugins ourselves.
...
Handwritten code requires manual work, and is therefore the most interesting criteria when we compare the code size of the plugins. However, the difference between the amount of generated code between the \gls{ANTLR} based Yan LR plugin ($1108$ code lines), and the Bison based YAMBi plugin ($2046$ code lines) is also interesting. One reason for the big difference might be that \gls{ANTLR} also requires a runtime library, while Bison generates all code needed for the parser. Both of these approaches have advantages and disadvantages. While Bisons’ approach means no additional dependencies, \gls{ANTLR}’s runtime library provides space advantages. If multiple programs on the same machine use an \gls{ANTLR} based parser, they can use the same compiled code, which only has to be stored in memory and on disk once.
...
The parser based plugin with the least amount of code is the \gls{ANTLR} based Yan LR. The main reason for this is, that \gls{ANTLR} already generates code for functionality that we had to create ourselves for the other plugins. While writing this support code is usually not that hard, it is certainly an advantage that \gls{ANTLR} already provides support for common tasks, such as tree walking.
...
\begin{figure}[H]
  \centering
    \includegraphics[width=0.7\textwidth]{StreamNullEOF}
...
We started the comparison by listing the basic error messages for the \glstext{YAML} plugins. These messages contain the error location and the auto-generated error text by the parsing engines. For the sake of brevity we removed data that is identical for all plugins, such as the filename of the parsed file.
...
\gls{ANTLR} uses an error listener class that provides a callback method that includes access to
...
for each detected error. As we already saw in Table~\ref{tab:error_messages_improved_list_element_outside}, the default error message provided by \gls{ANTLR} usually describes an error already well. For the initial version of the \href{http://libelektra.org/plugins/yanlr}{Yan LR plugin}, we only stored the last error message reported by \gls{ANTLR}. Since \gls{ANTLR} uses methods such as token deletion and insertion to keep parsing a file, even if it contains multiple errors~\cite{parr2013definitive}, the last error message usually will not provide the most obvious information on how to fix an error.
...
In the first improvement step for the error messages of the Bison parser, we defined alternative names for tokens, just as we did for Yan LR. Bison supports this feature directly, which means we did not have to write a script to replace the symbols in the generated parser code. After this update the error message from Table~\ref{tab:error_messages_improved_list_element_outside} changed from:
...
for the input shown in Listing~\ref{lst:incorrect_indentation_element_without_sequence}. As you can see above, we also added the erroneous input to the error message, just as we did in the Yan LR plugin.
...
\begin{itemize}
  ...
  \item \emph{YAy PEG} shows the correct line number for the first error, but not the correct column number. The plugin also only provides a very generic error message.
...
We conclude this section by answering~\Cref{que:message}.

\message*

While the parsing libraries do not produce particularly great error messages, at least the \gls{ANTLR} (Yan LR) and Bison (YAMBi) plugin, provide error messages that are comparable in quality to the ones of the handwritten parsing engine (YAML CPP).

One advantage of Yan LR, YAMBi, and YAwn is that their parsers offer error recovery. They are therefore able to report multiple errors in a file. This is something that YAML CPP is currently not able to do. \gls{ANTLR} offers error recovery for free, while Bison and YAEP require the addition of error tokens to the grammar. This can be problematic, since these error tokens can produce conflicts in the case of Bison, and ambiguous parsing results in the case of YAEP.

The parsing plugin that showed the least useful error messages is YAy PEG. While the \gls{PEGTL} offers basic error handling facilities that are able to provide good error messages for character level errors, producing good error messages for “high-level” errors would require a substantial amount of work.
...
\begin{table}[H]
  ...
  \begin{tabular}{p{2cm}p{2.3cm}p{2.3cm}p{2.3cm}p{2.3cm}p{2.3cm}}
    \toprule
    & \makecell{\href{https://github.com/jbeder/yaml-cpp}{yaml-cpp}} & \makecell{\href{http://antlr.org}{ANTLR}} & \makecell{\href{https://www.gnu.org/software/bison}{Bison}} & \makecell{\href{https://github.com/vnmakarov/yaep}{YAEP}} & \makecell{\href{https://github.com/taocpp/PEGTL}{PEGTL}}\\
    \midrule
    ...
    Input API (Encoding) & \makecell{\vspace{-0.4cm}{—}} & \makecell{\vspace{-0.4cm}{✅}} & \makecell{\vspace{-0.4cm}{❌}} & \makecell{\vspace{-0.4cm}{❌}} & \makecell{\vspace{-0.4cm}{✅}}\\
    Token Handling & \makecell{\vspace{-0.4cm}{—}} & \makecell{\vspace{-0.4cm}{✅}} & \makecell{\vspace{-0.4cm}{✅}} & \makecell{\vspace{-0.4cm}{❌}} & \makecell{\vspace{-0.4cm}{✅}}\\
    AST \newline Support & \makecell{\vspace{-0.4cm}{—}} & \makecell{\vspace{-0.4cm}{✅}} & \makecell{\vspace{-0.4cm}{❌}} & \makecell{\vspace{-0.4cm}{✅}} & \makecell{\vspace{-0.4cm}{✅}}\\
    Tree \newline Walking & \makecell{\vspace{-0.4cm}{—}} & \makecell{\vspace{-0.4cm}{✅}} & \makecell{\vspace{-0.4cm}{❌}} & \makecell{\vspace{-0.4cm}{❌}} & \makecell{\vspace{-0.4cm}{❌}}\\
    Tree \newline Rewriting & \makecell{\vspace{-0.4cm}{—}} & \makecell{\vspace{-0.4cm}{❌}} & \makecell{\vspace{-0.4cm}{❌}} & \makecell{\vspace{-0.4cm}{✅}} & \makecell{\vspace{-0.4cm}{✅}}\\
    Error \newline Listener & \makecell{\vspace{-0.4cm}{—}} & \makecell{\vspace{-0.4cm}{✅}} & \makecell{\vspace{-0.4cm}{❌}} & \makecell{\vspace{-0.4cm}{❌}} & \makecell{\vspace{-0.4cm}{❌}}\\
    Error \newline Recovery & \makecell{\vspace{-0.4cm}{❌}} & \makecell{\vspace{-0.4cm}{✅}} & \makecell{\vspace{-0.4cm}{✅\tnote{E}}} & \makecell{\vspace{-0.4cm}{✅\tnote{E}}} & \makecell{\vspace{-0.4cm}{❌}}\\
    Language Support &
    \begin{minipage}[t]{\linewidth}
    \begin{itemize}[leftmargin=*, itemsep=-1ex]
      \item C++
...
\begin{table}[H]
  ...
  \begin{tabular}{p{2.5cm}p{2.2cm}p{2.2cm}p{2.2cm}p{2.2cm}p{2.2cm}}
    ...
    Memory Usage\tnote{M} & 9.8 MB & 24.5 MB & 10.5 MB & 20.9 MB & 8.6 MB \\
    ...
    Time Frame\tnote{F}
    &
    \href{https://github.com/ElektraInitiative/libelektra/pull/1613/commits/b37cf6bedcf23fc3432c28491ec368981582440f}{6.8.2017}
    –
    \href{https://github.com/ElektraInitiative/libelektra/pull/1613}{10.10.2017}
    &
    \href{https://github.com/sanssecours/Yan-LR/commit/d6375c8a607aa38d78b092143ea143518d8e60e5}{6.2.2018}
    –
    \href{https://github.com/ElektraInitiative/libelektra/pull/2122}{27.7.2018}
    &
    \href{https://github.com/sanssecours/YAMBi/commit/86798f144381bac2b54bae228d95d7a9cf73b6a6}{25.8.2018}
    –
    \href{https://github.com/ElektraInitiative/libelektra/pull/2225}{10.9.2018}
    &
    \href{https://github.com/sanssecours/YAwn/commit/920f15e3933f7beca2d73ec0ab8863f00e459b3a}{11.9.2018}
    –
    \href{https://github.com/ElektraInitiative/libelektra/pull/2251}{9.10.2018}
    &
    \href{https://github.com/sanssecours/YAy-PEG/commit/25d80826e92bc1b0b4c389eaa5e61b7b98779ceb}{26.9.2018}
    –
    \href{https://github.com/ElektraInitiative/libelektra/pull/2314}{20.1.2019}
    \\
    \bottomrule
  \end{tabular}
  ...
  \begin{tablenotes}
    ...
    \item[F] The data in this row specifies the date from the initial commit of the respective plugin up to the first merged version of the plugin. The time frames do not include all the time necessary for research.
    ...
    \item[X] This line shows an overall rating according to the conclusion section about the respective feature.
  \end{tablenotes}
...
\begin{enumerate}
  ...
  \end{enumerate}
  In this scenario the \cpp{KeySet} read in step a) and step c) should be identical.
...
\begin{description}[style=multiline, leftmargin=2cm, font=\bfseries]
  \item[YAML CPP] The library used by the plugin (yaml-cpp) does currently not store comments (2. requirement) or line information (5. requirement). Adding this kind of information should be possible, but would require large modifications of yaml-cpp’s code base.

  \item[YAy PEG] The error message information provided by the plugin is quite bad and does therefore not meet requirement 4.
\end{description}
...
\begin{description}[style=multiline, leftmargin=2cm, font=\bfseries]
  \item[Yan LR] Compared to YAwn, the plugin performed worse in the runtime benchmark. It also requires more memory to parse our example data than YAMBi. On the other hand, \gls{ANTLR} provides a lot of support code we had to write for the other plugins ourselves.

  \item[YAMBi] The plugin performed well in the runtime performance test on Linux and – compared to Yan LR and YAwn – only required a relatively small amount of memory for our example data. The plugin does require more support code than Yan LR though. A problem considering the maintainability of the plugin might be the LR parsing algorithm. If there are any problems like shift/reduce conflicts in the grammar, then a developer usually needs at least some information on how the LR parsing algorithm works to fix the problem. Bison also offers considerable less support code for common parsing task than \gls{ANTLR} (see Table~\ref{tab:parser_overview}), which can be a problem, e.g.\  if we are not able to only use parser actions for an extension of the YAMBi plugin.

  \item[YAwn] While the plugin showed good runtime performance, we found two disadvantages, compared to YAMBi and Yan LR.

  \begin{enumerate}
    \item \gls{YAEP} is more or less the work of a single author: Vladimir N. Makarov. Recently another author, Alexander Klauer, fixed some of the problems of the project and modernized parts of the code base. However, compared to \gls{ANTLR} and Bison, the community behind the parsing library is rather small.

    \item YAEP requires more support code than Yan LR and even YAMBi. While this support code is not that complicated, it would still be something we need to maintain and extend for a more complete support of \glstext{YAML} and the requirements listed in “\nameref{sec:requirements_extended_yaml_plugin}”.
  \end{enumerate}

\end{description}

With all the information above in mind, we decided that the best candidate for extension is Yan LR. We think \gls{ANTLR}’s advantages, such as
...
make up for the worse runtime performance compared to \gls{YAEP}, and the relatively large heap memory usage compared to Bison.
