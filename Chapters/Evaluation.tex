...
\begin{figure}[H]
  \centering
    \includegraphics[width=0.6\textwidth]{StreamNullEOF}
...
You might also notice that in the error report \code{COMMENT} is missing from the list of expected \glspl{token}. This difference is the result of an \href{https://github.com/ElektraInitiative/libelektra/commit/0fe4953}{ambiguity in the ANTLR grammar we fixed}.
...
In the first improvement step for the error messages of the Bison parser, we defined alternative names for \glspl{token}, just as we did for Yan LR. Bison supports this feature directly, which means we did not have to write a script to replace the symbols in the generated parser code. After this update the error message from Table~\ref{tab:error_messages_improved_list_element_outside} changed from:
...
We then looked into the error recovery capabilities of Bison. Unlike \gls{ANTLR} the generated parser does not do error recovery by default, but rather exits on the first error. To improve the error behavior, Bison offers the possibility to add the predefined \code{error} \gls{token} to a grammar. Every time the Bison parser encounters an error it will produce this \gls{token}~\cite{donnelly2019bison}. We modified the grammar to allow errors inside \glstext{YAML} maps and sequences:
...
Just as Bison, \gls{YAEP} also requires that we add error \glspl{token} to the grammar to specify locations for error recovery. We therefore defined the same error recovery locations inside sequences and maps, as we did for Bison. The other updates were quite similar too: We improved the name of \glspl{token} inside error messages and added the erroneous input to the error message.
...
The only thing missing is the information about the expected type of \gls{token}.
...
One advantage of Yan LR, YAMBi, and YAwn is that their parsers offer error recovery. They are therefore able to report multiple errors in a file. This is something that YAML CPP is currently not able to do. \gls{ANTLR} offers error recovery for free, while Bison and YAEP require the addition of error \glspl{token} to the grammar. This can be problematic, since these error \glspl{token} can produce conflicts in the case of Bison, and ambiguous parsing results in the case of YAEP.
