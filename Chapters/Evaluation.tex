...
\begin{table}[H]
  ...
  \begin{tabular}{p{2cm}p{2.3cm}p{2.3cm}p{2.3cm}p{2.3cm}p{2.3cm}}
    ...
    \begin{itemize}[leftmargin=*, itemsep=-1ex]
      \item Parser Actions
      \item AST
      \item Tree \newline Walking (Listener / Visitor)
    \end{itemize}
    \end{minipage}
    &
    \begin{minipage}[t]{\linewidth}
...
\begin{table}[H]
  ...
  \begin{tabular}{p{2cm}p{2.3cm}p{2.3cm}p{2.3cm}p{2.3cm}p{2.3cm}}
    ...
    \midrule
    Parser \newline Technique & Recursive Descent & \gls{ALL(*)} & \glstext{LALR}(1) & Earley Parser & PEG Parser\\
    ...
    Run Time\tnote{R} & 188.9 & 122 & 119.9 & 74.9 & 261 \\
  ...
  \begin{tablenotes}
    ...
    \item[L] This number includes the line number of the grammar, but not any generated code, according to the data shows in Table~\ref{fig:line_count}.
    \item[R] This row contains the mean of the execution times in milliseconds for the file \FileGenerated{} show in Table~\ref{fig:benchmark_generated_mean}.
...
make up for the worse runtime performance on Linux and the relatively large heap memory usage compared to Bison and YAEP.
